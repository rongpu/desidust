...
catn = Table(fitsio.read('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/rz_corrected/rz_north.fits'))
catn1 = Table(fitsio.read('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/rz_corrected/rz_north_predicted_error.fits'))
assert len(catn)==len(catn1) and np.all(catn['TARGETID_DR9']==catn1['TARGETID_DR9'])
catn = hstack([catn, catn1[['rz_err']]])
cats = Table(fitsio.read('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/rz_corrected/rz_south.fits'))
cats1 = Table(fitsio.read('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/rz_corrected/rz_south_predicted_error.fits'))
assert len(cats)==len(cats1) and np.all(cats['TARGETID_DR9']==cats1['TARGETID_DR9'])
cats = hstack([cats, cats1[['rz_err']]])

catn['drz_north'] = catn['drz'].copy()  # reddening measurement with Northern imaging

# Convert to Southern reddening measurement: remove offset and renormalize
catn['drz'] = (catn['drz_north']-north_rz_offset)/(r['r_north']-r['z_north'])*(r['r_south']-r['z_south'])

cats['drz_north'] = np.nan
...
cat.write('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/rz_corrected/rz_combined.fits', overwrite=False)

############################################################################################################################

# cat = Table(fitsio.read('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/rz_corrected/rz_combined.fits'))
...
for nside in [64, 128, 256, 512]:
    ...
    maps.write('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/maps/drz_map_combined_{}.fits'.format(nside), overwrite=False)
