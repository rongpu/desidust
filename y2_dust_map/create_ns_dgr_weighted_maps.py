...
for field in ['north', 'south']:
    ...
    cat = Table(fitsio.read('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/gr_corrected/gr_{}.fits'.format(field)))
    cat1 = Table(fitsio.read('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/gr_corrected/gr_{}_predicted_error.fits'.format(field)))
    assert len(cat)==len(cat1) and np.all(cat['TARGETID_DR9']==cat1['TARGETID_DR9'])
    cat = hstack([cat, cat1[['gr_err']]])
    ...
    for nside in [64, 128, 256, 512]:
        ...
        maps.write('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/maps/dgr_map_{}_{}.fits'.format(field, nside), overwrite=False)
