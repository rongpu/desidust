# Create (mostly) unbiased reference catalog for r-z correction
...
for field in ['south', 'north']:
    ...
    for iteration in range(7):
        ...
        if iteration==0:
            ...
            cat_ref['drz_ref'] = cat_ref['drz_sfd_raw'].copy()  # offset in delta_r-z
        else:
            maps = Table(fitsio.read('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/tmp/drz_map_{}_{}_reference_{}.fits'.format(field, nside, iteration-1)))
            mask = maps['n_star']>=16
            ...
            cat_ref = join(cat_ref, maps[['delta_rz_mean', 'HPXPIXEL']], keys='HPXPIXEL', join_type='inner')
            cat_ref['drz_ref'] = cat_ref['drz_raw'] - cat_ref['delta_rz_mean']  # offset in delta_r-z
        ...
        maps.write('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/tmp/drz_map_{}_{}_reference_{}.fits'.format(field, nside, iteration), overwrite=True)

    #################### Create reference catalog ########################

    # maps = Table(fitsio.read('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/tmp/drz_map_{}_{}_reference_{}.fits'.format(field, nside, iteration)))
    mask = maps['n_star']>=16
    print('n_star', np.sum(~mask)/len(mask))
    maps = maps[mask]
    ...
    cat_ref = cat.copy()
    cat_ref['HPXPIXEL'] = hp.ang2pix(nside, cat_ref['TARGET_RA'], cat_ref['TARGET_DEC'], nest=False, lonlat=True)
    cat_ref = join(cat_ref, maps[['delta_rz_mean', 'delta_rz_median', 'delta_rz_hlmean', 'n_star', 'HPXPIXEL']], keys='HPXPIXEL', join_type='inner')
    cat_ref['drz_ref'] = cat_ref['drz_raw'] - cat_ref['delta_rz_mean']  # offset in delta_r-z

    cat_ref.write('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/rz_corrected/rz_reference_{}.fits'.format(field), overwrite=True)
