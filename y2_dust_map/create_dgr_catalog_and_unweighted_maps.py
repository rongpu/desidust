...
# Create delta_g-r catalog with correction based on the reference catalog (and independent from SFD)
...
# gr_offset_dict = {'dark_std': -2.9, 'bright_std': -3.3, 'bright_mws': 0.0, 'backup_std': -1.0, 'backup_mws': -0.9}  # per_star_type_gr_offsets.ipynb (in mmag)
...
for field in ['south', 'north']:
    ...
    cat_ref = Table(fitsio.read('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/gr_corrected/gr_reference_{}.fits'.format(field)))
    ...
    #     dgr_new[mask] = cat['dgr'][mask] - 1e-3*gr_offset_dict[star_type]
    # assert np.all(dgr_new!=-99.)
    # cat['dgr'] = dgr_new

    cat.write('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/gr_corrected/gr_{}.fits'.format(field), overwrite=True)
    ...
    for nside in [64, 128, 256]:
        ...
        maps.write('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/maps/dgr_map_{}_{}.fits'.format(field, nside), overwrite=True)
