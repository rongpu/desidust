...
cat = Table(fitsio.read('/dvs_ro/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/gr_corrected/gr_{}.fits'.format(field)))
print(len(cat))
...
maps = Table(fitsio.read('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/maps/dgr_map_{}_{}.fits'.format(field, nside)))
...
cat = join(cat, maps[['delta_gr_mean', 'delta_gr_hlmean', 'HPXPIXEL', 'n_star']], keys='HPXPIXEL', join_type='inner')
...
cat['dgr_diff_raw'] = cat['dgr_raw'] - cat['delta_gr_mean']
cat['dgr_diff'] = cat['dgr'] - cat['delta_gr_mean']
...
for star_type in ['dark_std', 'bright_std', 'bright_mws', 'backup_std', 'backup_mws']:
    ...
    # compute the average g-r error in 2D bins of SN_B and model_gr_std; count the number of objects in each bin
    xbins, ybins = np.linspace(0, 80, 81), np.linspace(0.01, 0.031, 71)
    ret = binned_statistic_2d(cat['SN_B'][mask0], cat['model_gr_std'][mask0], cat['dgr_diff'][mask0], statistic=rms, bins=[xbins, ybins])
    count = binned_statistic_2d(cat['SN_B'][mask0], cat['model_gr_std'][mask0], None, statistic='count', bins=[xbins, ybins])

    # get the binned_statistic_2d bin indices
    # (for determining which regions in the SN_B and model_gr_std are removed by the minimum count cut)
    ...
    count1 = binned_statistic_2d(cat['SN_B'], cat['model_gr_std'], cat['dgr_diff'], statistic='count', bins=[xbins, ybins])
    mask1 = mask0 & np.in1d(count1.binnumber, bin_idx)
    print('Outliers in SN_B vs model_gr_std: {:.2f}%'.format(100*np.sum(~mask1 & mask0)/np.sum(mask0)))

    # compute the error for the well-sampled regions in SN_B vs model_gr_std space
    count_all = binned_statistic_2d(cat_all['SN_B'], cat_all['model_gr_std'], None, statistic='count', bins=[xbins, ybins])
    good_region = cat_all[star_type] & np.in1d(count_all.binnumber, bin_idx)
    cat_all['gr_err'][good_region] = np.clip(poly_val2d(cat_all['SN_B'][good_region], cat_all['model_gr_std'][good_region], coeffs), min_gr_err, np.inf)

    # create nearest-neighbor training data
    x1, x2 = cat_all['SN_B'].copy(), cat_all['model_gr_std'].copy()
    ...
    X_outliers = np.vstack([cat_all['SN_B'][mask]/x1_sigma, cat_all['model_gr_std'][mask]/x2_sigma]).T
    idx = nbrs.kneighbors(X_outliers)[1]
    cat_all['gr_err'][mask] = cat_all['gr_err'][good_region][idx[:, 0]]
    assert np.sum(cat_all['gr_err'][cat_all[star_type]]==0)==0
    ...
    ax.set_xlabel('SN_B')
    ax.set_ylabel('model_gr_std')
    fig.colorbar(pcm, ax=ax)
    ax = axes[1]
    pcm = ax.imshow(rms_data.reshape(ret.statistic.T.shape), origin='lower', extent=[xbins.min(), xbins.max(), ybins.min(), ybins.max()], aspect='auto', vmin=0.01, vmax=0.03, cmap='jet')
    ax.set_xlabel('SN_B')
    ax.set_ylabel('model_gr_std')
    fig.colorbar(pcm, ax=ax)
    plt.show()
    ...
    fig.colorbar(pcm, ax=ax)
    ax = axes[1]
    pcm = ax.imshow(rms_predict.reshape(ret.statistic.T.shape), origin='lower', extent=[xbins.min(), xbins.max(), ybins.min(), ybins.max()], aspect='auto', vmin=0.01, vmax=0.03, cmap='jet')
    ax.set_xlabel('SN_B')
    ax.set_ylabel('model_gr_std')
    fig.colorbar(pcm, ax=ax)
    plt.show()

    count2 = binned_statistic_2d(cat['SN_B'][mask1], cat['model_gr_std'][mask1], None, statistic='count', bins=[xbins, ybins])
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    ax = axes[0]
    ...
    ret1 = binned_statistic_2d(cat_all['SN_B'][mask], cat_all['model_gr_std'][mask], cat_all['gr_err'][mask], statistic=np.mean, bins=[xbins, ybins])
    pcm = ax.imshow(ret1.statistic.T, origin='lower', extent=[xbins.min(), xbins.max(), ybins.min(), ybins.max()], aspect='auto', vmin=0.01, vmax=0.03, cmap='jet')
    ax.set_xlabel('SN_B')
    ax.set_ylabel('model_gr_std')
    fig.colorbar(pcm, ax=ax)
    plt.show()
    ...
    #              range=(np.nanmin(rms_data), np.nanmax(rms_data)), alpha=0.5, histtype='step', color='r', lw=1)
             range=(np.nanmin(rms_data), 0.03), alpha=0.5, histtype='step', color='r', lw=1)
    plt.show()

assert np.sum(cat_all['gr_err']==0)==0

print(len(cat_all), len(np.unique(cat_all['TARGETID_DR9'])), len(cat_all)==len(np.unique(cat_all['TARGETID_DR9'])))

cat_all = cat_all[['TARGETID_DR9', 'gr_err']]
cat_all.write('/global/cfs/cdirs/desicollab/users/rongpu/data/ebv/desi_stars/gr_corrected/gr_{}_predicted_error.fits'.format(field), overwrite=True)
